...
The **Add User Journey** outlines the step-by-step process for adding a new user to the system. This involves logging in, filling in user details, defining roles and permissions, and going through an approval process via the **Maker-Checker-Banker-Approver** flow which is **configurable** based on business needs. The journey ensures that all required fields are filled out, the user's access is configured properly, and that appropriate checks are conducted for security and compliance purposes.
