...
The View User Journey defines the steps involved in reviewing an existing user’s details within the system. This process allows the congfigured BPM flow **Admin-Checker-Banker-Approver** or relevant user to inspect user data, role assignments, permissions, and other attributes related to the user’s profile. The journey ensures that the user’s data is accurate and up-to-date.
